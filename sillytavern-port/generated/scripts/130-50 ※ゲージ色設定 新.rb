class Game_Battler < Game_BattlerBase
  ...
  def ap_gauge_color_nil  # 詠唱中でない時のＡＰゲージ色
    ...
    # 基本色
    color[0] = Color.new( 32,  32,  64)   # text_color(19)と同じ
    color[1] = Color.new(160,  96, 224)   # text_color(30)
    color[2] = Color.new(192, 128, 255)   # text_color(31)
    
    if cst(27, 29)  # ステート27か29が付加されている
      color[1] = Color.new(240, 192,  64)
      color[2] = Color.new(255, 255,  64)
    end
    
    if cst(40)      # ステート40　狂戦士化
      color[1] = Color.new(240, 192,  64)
      color[2] = Color.new(255, 255,  64)
    elsif cst(44)   # ステート44　魔道の真髄(40が付加されていない時のみ)
      color[1] = Color.new(226, 266,   0)
      color[2] = Color.new(164, 245,   0)
    elsif cst(37)   # ステート37  スロウ(40,44が付加されていない時のみ)
  ...
  def ap_gauge_color_1  # 詠唱タイプ１の時のゲージ色
    ...
    if cst(28, 29)  # ステート28か29
      color[1] = Color.new(226, 266,   0)
      color[2] = Color.new(164, 245,   0)
    end
    
    # ステート40（狂戦士化）の間は詠唱できないのでここに設定する必要はない
    if cst(37)        # ステート37  スロウ
  ...
  def ap_gauge_color_2  # 詠唱タイプ２の時のゲージ色
    ...
    if cst(44)      # ステート44　魔道の真髄
      color[1] = Color.new(226, 266,   0)
      color[2] = Color.new(164, 245,   0)
    elsif cst(37)   # ステート37  スロウ
