...
Browser acceptance exercised 35 seconds of real input over Map98 with horizontal, vertical, diagonal, normal, and dash movement: 1,638 presented frames, 278 camera positions, zero invalid tile lookups, zero missing-tile samples, and zero black-hole frames. A separate real-data fixture placed the player at `(7,19)` and hostile Event 16 at `(4,19)`; the enemy detected, chased two tiles, contacted the player, and entered troop 3 `Lợn Đồ Tể`. The player guarded, the enemy attacked for 613 damage, and the player attacked back for 200 damage to finish the battle. Original `Data/`, `Graphics/`, `Audio/`, `System/`, `Game.exe`, and `Game.ini` have no Git diff.
