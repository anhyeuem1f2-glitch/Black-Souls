# Custom Script Coverage

Generated from the decompressed `Data/Scripts.rvdata2` archive.

- Total entries: 167
- Custom/plugin entries: 57
- Custom/plugin source: 16411 lines / 667312 bytes
- Static `Win32API` references across all scripts: 0

| Index | Script | Lines | Bytes | Win32API refs | Browser status |
|---:|---|---:|---:|---:|---|
| 109 | 入手インフォ | 604 | 25465 | 0 | not ported |
| 110 | セルフスイッチ操作 | 149 | 5707 | 0 | not ported |
| 111 | メニュー変数 | 90 | 3209 | 0 | not ported |
| 112 | 特殊軽量化 | 38 | 1450 | 0 | not ported |
| 113 | A1共通スクリプト | 1094 | 50161 | 0 | not ported |
| 114 | 装備拡張 | 287 | 12914 | 0 | not ported |
| 115 | アクター窓 | 52 | 2610 | 0 | not ported |
| 116 | 画面外自律移動 | 80 | 3510 | 0 | not ported |
| 117 | ステータス限界突破 | 265 | 8163 | 0 | not ported |
| 118 | タイマー非表示 | 68 | 2520 | 0 | not ported |
| 119 | ８方向移動 | 152 | 5645 | 0 | not ported |
| 120 | アイテム合成 | 889 | 33951 | 0 | not ported |
| 121 | バトルスピード調整 | 207 | 9466 | 0 | not ported |
| 122 | 61 ※ＡＴＢ　メイン設定 | 125 | 4429 | 0 | not ported |
| 123 | 71 Scene_Battle | 297 | 12128 | 0 | not ported |
| 124 | 62 ステート　ターンフレーム | 362 | 14983 | 0 | not ported |
| 125 | 62 Game_Battler | 276 | 10486 | 0 | not ported |
| 126 | 71 Game_Battler フレーム更新 | 87 | 3022 | 0 | not ported |
| 127 | 71 Game_Battler 詠唱 | 200 | 8050 | 0 | not ported |
| 128 | 71 ※ＡＰ増減ステート　設定 | 257 | 9986 | 0 | not ported |
| 129 | 64 ※ＡＴＢ　ウインドウ設定 | 93 | 4529 | 0 | not ported |
| 130 | 50 ※ゲージ色設定　新 | 132 | 4900 | 0 | not ported |
| 131 | 戦闘コマンドカスタム1.05 | 952 | 39926 | 0 | not ported |
| 132 | 70 ステータスウインドウ | 285 | 11511 | 0 | not ported |
| 133 | 61 敵ＡＰゲージ描画 | 298 | 11834 | 0 | not ported |
| 134 | 敵のHP表示 | 631 | 23779 | 0 | not ported |
| 135 | 根性食いしばり | 74 | 3137 | 0 | not ported |
| 136 | 敵の行動賢く | 608 | 29623 | 0 | not ported |
| 137 | 戦闘背景変更 | 87 | 4122 | 0 | not ported |
| 138 | EX WINDOW | 242 | 11287 | 0 | not ported |
| 139 | 顔グラ戦闘 | 88 | 3752 | 0 | not ported |
| 140 | VXAce_SP1 | 158 | 7562 | 0 | not ported |
| 141 | 逃走確率 | 91 | 3371 | 0 | not ported |
| 142 | 遠景床 | 54 | 2687 | 0 | not ported |
| 143 | 歩数リセット | 3 | 58 | 0 | not ported |
| 144 | 名前入力 | 1053 | 42509 | 0 | not ported |
| 145 | シンボル | 680 | 29739 | 0 | not ported |
| 146 | 自動蘇生 | 555 | 17978 | 0 | not ported |
| 147 | HP回復反転 | 281 | 9853 | 0 | not ported |
| 148 | リージョン通行 | 340 | 14654 | 0 | not ported |
| 149 | 戦闘終了時戦闘不能解除 | 195 | 5924 | 0 | not ported |
| 150 | 誤選択防止 | 114 | 4096 | 0 | not ported |
| 151 | hulogu | 339 | 13418 | 0 | not ported |
| 152 | バトラー表示拡張 | 106 | 4217 | 0 | not ported |
| 153 | バトルミスト | 110 | 4272 | 0 | not ported |
| 154 | クリティカル効果音 | 44 | 1671 | 0 | not ported |
| 155 | 使用者効果 | 43 | 1597 | 0 | not ported |
| 156 | ネームウィンドウ | 765 | 29433 | 0 | not ported |
| 157 | 全体攻撃武器 | 144 | 5452 | 0 | not ported |
| 158 | マップ名ウィンドウ | 79 | 3065 | 0 | not ported |
| 159 | 足音 | 129 | 6507 | 0 | not ported |
| 160 | レベル限界突破 | 339 | 12219 | 0 | not ported |
| 161 | バトルシェイク | 106 | 3921 | 0 | not ported |
| 162 | 周回敵の強さ | 123 | 6418 | 0 | not ported |
| 163 | King Maker worldmap | 515 | 13751 | 0 | not ported |
| 164 | King Maker walkthrough | 888 | 43372 | 0 | not ported |
| 165 | メッセージスキップ | 88 | 3313 | 0 | not ported |
